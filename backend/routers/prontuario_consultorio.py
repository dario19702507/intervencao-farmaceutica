    ...
    if not medicamentos:
        ...
        for m in medicamentos:
            adicionar_tabela([
                ...
                ["Uso off-label", "Sim" if getattr(m, "uso_off_label", "NAO_AVALIADO") == "SIM" else ("Não" if getattr(m, "uso_off_label", "NAO_AVALIADO") == "NAO" else "Não avaliado")],
                ["Justificativa off-label", texto(getattr(m, "justificativa_off_label", None))],
                ["Evidência/referência", texto(getattr(m, "evidencia_off_label", None))],
