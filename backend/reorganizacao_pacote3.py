...
backup_dir = BASE / 'backup_reorganizacao_pacote3'
backup_dir.mkdir(exist_ok=True)

consultorio_path = routers_dir / 'consultorio.py'
main_path = BASE / 'main.py'
pacientes_path = routers_dir / 'pacientes.py'

if not consultorio_path.exists():
    raise FileNotFoundError('Não encontrei routers/consultorio.py. Execute este script dentro da pasta backend.')

# Backup
stamp = datetime.now().strftime('%Y%m%d_%H%M%S')
(backup_dir / f'consultorio_{stamp}.py').write_text(consultorio_path.read_text(encoding='utf-8'), encoding='utf-8')
if main_path.exists():
    (backup_dir / f'main_{stamp}.py').write_text(main_path.read_text(encoding='utf-8'), encoding='utf-8')
...
pacientes_path.write_text(pacientes_code.strip() + '\n', encoding='utf-8')

# Atualizar main.py para incluir router de pacientes, se ainda não estiver incluído.
if main_path.exists():
    main_text = main_path.read_text(encoding='utf-8')
    if 'from routers.pacientes import router as pacientes_router' not in main_text:
    ...
    if 'app.include_router(pacientes_router)' not in main_text:
        main_text = main_text.replace(
...
print('Pacote 3 aplicado.')
print('Criado/atualizado: routers/pacientes.py')
print('Atualizado: main.py com pacientes_router')
print('Backup em:', backup_dir)
